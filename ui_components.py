...
def render_top20_card(s):
    ...
    <div style="border: 2px solid {color}; border-radius: 10px; padding: 18px; 
        ...
        <div style="display: flex; justify-content: space-between;">
            <div><span style="color: gray; font-size: 13px;">勝率</span><br>
                <b style="font-size: 22px; color: {color};">{prob:.1f}%</b></div>
            <div style="text-align: right;"><span style="color: gray; font-size: 13px;">進場</span><br>
...
def render_single_diagnostic_card(long_prob, short_prob, signal, entry_price, res_level, sup_level):
    ...
    if signal in ["STRONG_LONG", "LONG"]:
        box_color, rec = "#00cc96", "⭐⭐⭐ 強勢做多" if signal == "STRONG_LONG" else "⭐⭐ 偏多佈局"
    elif signal in ["STRONG_SHORT", "SHORT"]:
        box_color, rec = "#ff4b4b", "⚠️⚠️ 強勢放空" if signal == "STRONG_SHORT" else "⚠️ 偏空操作"
    elif signal == "HIGH_VOLATILITY":
        box_color, rec = "#ffa500", "⚡ 多空雙巴 (建議空手)"
    else:
        box_color, rec = "#a8a8a8", "⚪ 動能不足 (建議觀望)"
    ...
    with col1:
        ...
        <div style='background:#1e1e1e; padding:12px; border-left:4px solid #00cc96; border-radius:5px; margin-bottom:8px;'>
            <div style='display:flex;justify-content:space-between;align-items:center;'>
                <div><span style='color:#aaa;font-size:11px;'>LightGBM</span><br><span style='font-size:22px; color:{c_l}; font-weight:bold;'>{wr_l*100:.1f}%</span></div>
                <div style='text-align:right;'><span style='color:#666;font-size:10px;'>{date_l}</span></div>
            </div>
        </div>""", unsafe_allow_html=True)
        ...
        <div style='background:#1e1e1e; padding:12px; border-left:4px solid #00cc96; border-radius:5px;'>
            <div style='display:flex;justify-content:space-between;align-items:center;'>
                <div><span style='color:#aaa;font-size:11px;'>LSTM</span><br><span style='font-size:22px; color:{c_ll}; font-weight:bold;'>{wr_ll*100:.1f}%</span></div>
                <div style='text-align:right;'><span style='color:#666;font-size:10px;'>{date_ll}</span></div>
            </div>
        </div>""", unsafe_allow_html=True)
    ...
    with col2:
        ...
        if wr_s > 0:
            ...
            <div style='background:#1e1e1e; padding:12px; border-left:4px solid #ff4b4b; border-radius:5px; margin-bottom:8px;'>
                <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <div><span style='color:#aaa;font-size:11px;'>LightGBM</span><br><span style='font-size:22px; color:{c_s}; font-weight:bold;'>{wr_s*100:.1f}%</span></div>
                    <div style='text-align:right;'><span style='color:#666;font-size:10px;'>{date_s}</span></div>
                </div>
            </div>""", unsafe_allow_html=True)
        else:
        ...
        if wr_ls > 0:
            ...
            <div style='background:#1e1e1e; padding:12px; border-left:4px solid #ff4b4b; border-radius:5px;'>
                <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <div><span style='color:#aaa;font-size:11px;'>LSTM</span><br><span style='font-size:22px; color:{c_ls}; font-weight:bold;'>{wr_ls*100:.1f}%</span></div>
                    <div style='text-align:right;'><span style='color:#666;font-size:10px;'>{date_ls}</span></div>
                </div>
            </div>""", unsafe_allow_html=True)
        else:
