# indicators.py — 修正版
# 修正點：
#   1. Score 矛盾：流動性掠奪(Liquidity_Sweep_Bull)通常伴隨爆量，
#      原本「爆量懲罰」與「流動性掠奪加分」互相抵消，現在分開處理。
#   2. ATR_14 不再寫死為 Close * 0.03，由 get_kline_with_fugle 動態傳入。
#   3. add_advanced_indicators 回傳時確保 ATR_14 欄位一定存在。
...
def add_advanced_indicators(df, market_ret_20=None):
    ...
    # ── 8. 潛伏型 AI 評分矩陣 (滿分 100) — 修正版 ────────────────────────
    #
    # 修正邏輯：
    #   原版「量 > Vol_SMA5 * 2.5 扣 15 分」會懲罰流動性掠奪訊號（後者本就爆量），
    #   兩個條件幾乎互相抵消。
    #   修正方案：
    #     ① 「流動性掠奪」出現時，爆量懲罰豁免（用 ~Liquidity_Sweep_Bull 判斷）
    #     ② 「追高」懲罰保留（非機構掃低點的飆漲仍需懲罰）
    #
    df['Score'] = 0
    ...
    # 懲罰追高（非流動性掠奪的單日急漲）
    is_chase = (
        ((df['Close'] - prev_close) / prev_close > 0.05) &
        (~df['Liquidity_Sweep_Bull'])
    )
    df.loc[is_chase, 'Score'] -= 15

    # 懲罰爆量（排除流動性掠奪情境，因為那是機構行為）
    is_vol_spike_bad = (
...
def add_intraday_indicators(df):
    ...
    df['ATR_14_1h'] = tr_h.rolling(14).mean()
