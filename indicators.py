...
def add_advanced_indicators(df, market_df=None):
    ...
    if df.empty or len(df) < 40: # 提高歷史 K 線要求以計算背離
        return df
    ...
    # 2. 🚀 新增：RSI 動態波段背離掃描 (Divergence Scanner)
    # 計算近 5 日波谷與前波段 (5~20日前) 波谷比對
    ...
    # 底背離 (Bullish): 股價創低，但 RSI 未破底且在低檔 (<45)
    ...
    # 頂背離 (Bearish): 股價創高，但 RSI 未過高且在高檔 (>55)
    ...
    # 6. 🏆 多因子 AI 評分系統 (滿分 100) - 加入背離權重
    df['Score'] = 0
    df.loc[df['Close'] > df['SMA_20'], 'Score'] += 10                  # 趨勢防護 (10)
    df.loc[df['Volume'] > df['Vol_SMA5'] * 1.5, 'Score'] += 10         # 量能發動 (10)
    df.loc[df['MACD'] > df['Signal'], 'Score'] += 10                   # 動能金叉 (10)
    df.loc[df['RS_Index'] > 0.02, 'Score'] += 15                       # 領漲抗跌 (15)
    df.loc[df['Weekly_Trend_Up'] == True, 'Score'] += 15               # 週線共振 (15)
    df.loc[(df['Squeeze_On'].shift(1) == True) & (df['Close'] > df['BB_Upper']), 'Score'] += 15 # 壓縮突破 (15)
    df.loc[df['Smart_Money_Trend'] >= 1, 'Score'] += 10                # 籌碼動能 (10)
    df.loc[df['Bullish_Div'] == True, 'Score'] += 15                   # 🚨 底部背離發動 (15)
