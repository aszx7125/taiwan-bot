...
def trigger_github_workflow(workflow_filename):
    ...
    if not token:
        return False, "缺少 GitHub PAT 金鑰設定"
    ...
    try:
        ...
        try:
            ...
            if os.path.exists("lstm_momentum_brain.h5") and os.path.exists("lstm_scaler.joblib"):
                lstm_model = tf.keras.models.load_model("lstm_momentum_brain.h5", compile=False)
        ...
        df['return_5d'] = (df['future_close_6d'] - df['entry_price_real']) / df['entry_price_real'].replace(0, np.nan)
        ...
        input_df = input_df.replace([np.inf, -np.inf], 0).clip(-1e6, 1e6)
        ...
        if use_lstm:
            ...
            for ticker, group in df.groupby('ticker'):
                ...
                preds = np.nan_to_num(preds, nan=0.5, posinf=0.99, neginf=0.01)
