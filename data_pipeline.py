    ...
    if not rt_price or rt_price == 0:
        try:
            ...
            if not df.empty and len(df) >= 2:
                rt_price = float(df.iloc[-1]['Close'])
                prev_close = float(df.iloc[-2]['Close'])
                rt_vol = float(df.iloc[-1].get('Volume', 0.0))
        except Exception: pass
...
def load_model_metrics():
    ...
    default_metrics = {
        ...
        "short": {"lgbm": {"blind_win_rate": 0.0, "last_train": "尚未訓練"}, "lstm": {"blind_win_rate": 0.0, "last_train": "尚未訓練"}}
    }
...
def blend_model_probs(lgbm_prob: float, lstm_prob: float, lgbm_weight: float = 0.6, lstm_weight: float = 0.4) -> float:
...
def fetch_advanced_backtest(ai_prob_threshold=0.50, use_market_filter=True, initial_cap=1000000, max_pos=5):
    try:
        ...
        df.replace([np.inf, -np.inf], 0, inplace=True) # 🔥 無限大防護
        ...
        if use_lstm:
            TIME_STEPS = 10
            feature_cols = ['daily_return', 'vol_ratio', 'broker_conc', 'rs_index', 'volatility', 'turnover', 'is_pullback', 'is_squeeze', 'is_divergence', 'is_liquidity_sweep', 'is_poc_rejection']
            ...
            for ticker, group in df.groupby('ticker'):
                ...
                X_seq = np.array([feat_arr[i:i+TIME_STEPS] for i in range(len(feat_arr) - TIME_STEPS + 1)], dtype=np.float32)
            ...
            df['final_prob'] = df.apply(lambda r: blend_model_probs(r['ai_prob'], r['lstm_prob']), axis=1)
        else:
        ...
        base_mask = (df['final_prob'] >= ai_prob_threshold) & (df['entry_price_real'].notna()) & (df['future_close_6d'].notna())
        signals = df[base_mask & (df['market_close'] >= df['market_sma20'])].copy() if use_market_filter else df[base_mask].copy()
        ...
        for current_date, daily_sigs in signals.groupby('date_norm'):
            ...
            daily_equity.append({'date_str': date_str, 'strat_cum_pct': ((current_equity - initial_cap) / initial_cap) * 100, 'market_cum_pct': market_lookup.get(date_str, 0.0)})
        ...
        return {
            "status": "ready",
            "ai_strat": {"wr": wins / total_samples if total_samples > 0 else 0, "w": wins, "l": total_samples - wins},
            ...
            "signals": exec_df.sort_values('date', ascending=False).head(50)[['date', 'ticker', 'entry_price_real', 'final_prob', 'sim_profit_twd']].to_dict('records'),
            "equity": daily_equity
        }
