# app.py — 完整修正版
# 修正點：
#   1. get_market_summary 加入 @st.cache_data(ttl=300) 防止重複 fetch
#   2. 貪婪指數改為多維度計算：
#      大盤漲跌幅 + 全市場平均勝率 + 平均 RS_Index + 平均量比
#   3. 移除 random.randint，改為確定性計算
#   4. 新增貪婪指數視覺化儀表盤（顏色 + 標籤）
...
def compute_fear_greed(twii_pct: float, snapshot_data: list) -> tuple[int, str, str]:
    ...
    多維度貪婪指數計算（0-100）。
    維度：
      1. 大盤漲跌幅   (權重 35%)：pct 對應 -3%~+3% → 0~100
      2. AI 全市場多空比例 (權重 35%)：勝率 > 50% 的標的佔比
      3. 平均 RS_Index (權重 20%)：相對強弱均值
      4. 平均量比     (權重 10%)：vol_ratio 均值，>1.2 偏多
    回傳 (index_0_100, label, color)
    """
    # ── 維度 1：大盤漲跌幅 ──────────────────────────────────────────────
    # pct 範圍 -3% ~ +3% 對應 0 ~ 100，超出邊界 clip
    pct_score = float(np.clip(50 + (twii_pct * 16.67), 0, 100))

    # ── 維度 2~4：從快取數據計算 ─────────────────────────────────────────
    bull_ratio_score = 50.0   # 預設中性
    rs_score         = 50.0
    vol_score        = 50.0
    ...
    if snapshot_data and len(snapshot_data) > 0:
        ...
        # 維度 2：AI 多空比（用 rs_index > 0 當多空判斷代理，因 ai_prob 不在快取裡）
        if 'rs_index' in df.columns:
            df['rs_index'] = pd.to_numeric(df['rs_index'], errors='coerce').fillna(0)
            bull_ratio = float((df['rs_index'] > 0).mean())   # 0~1
            bull_ratio_score = bull_ratio * 100                # 0~100

        # 維度 3：RS_Index 均值（通常 -10 ~ +10，clip 至 0~100）
        if 'rs_index' in df.columns:
            mean_rs = float(df['rs_index'].mean())
            rs_score = float(np.clip(50 + (mean_rs * 5), 0, 100))

        # 維度 4：量比均值（1.0 = 中性 → 50 分，>1.5 偏多，<0.7 偏空）
...
def render_fear_greed_gauge(index_val: int, label: str, color: str):
    ...
    <div style="background:#1e1e1e; border-radius:12px; padding:18px; text-align:center; border:1px solid #333;">
        ...
        <div style="font-size:38px; font-weight:700; color:{bar_color};">{index_val}</div>
        <div style="font-size:16px; color:{bar_color}; font-weight:600; margin-bottom:12px;">{label}</div>
        <div style="background:#333; border-radius:8px; height:10px; width:100%; overflow:hidden;">
            <div style="background:{bar_color}; width:{bar_pct}%; height:100%; border-radius:8px; transition:width 0.6s;"></div>
        </div>
    ...
    with tab1:
        ...
        def render_rt():
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as ex:
                ...
                for future in concurrent.futures.as_completed(future_to_ticker):
                    try:
                        ...
                        if res:
                            rows.append(res)
                        time.sleep(0.3)   # 修正：從 0.05 拉長至 0.3，配合限流修正
                    except Exception:
                        pass
            ...
            if rows:
                ...
                css = (
                    f"<style>.watch-board table{{width:100%!important;border-collapse:collapse;}}"
                    ...
                    f"vertical-align:middle!important;}}</style>"
                )
            ...
            for item in raw_list:
                ...
                if ep > 0:
                    bulk_features.append(
                        brain.extract_features(ticker, ep, snap_dict,
            ...
            if valid_items:
                ...
                for idx, item in enumerate(valid_items):
                    ...
                    if prob >= 0.50:
                        ...
                        processed.append({
                            'ticker': item.get('代號'), 'name': item.get('名稱', ''),
                            'win_prob': prob,
                            'box_color': "#00cc96" if prob >= 0.52 else "#ffc107",
                            'ai_rec':   "推薦佈局" if prob >= 0.52 else "謹慎試單",
                            'entry_price': ep, 'take_profit': tp,
                            'stop_loss': sl,   'profit_reason': "波段"
                        })
                ...
                for idx, (sub_name, tickers) in enumerate(chain_data.items()):
                    with cols[idx]:
                        ...
                        if sub_items:
                            ...
                            st.dataframe(
                                display_df.drop(columns=['_prob_raw']).sort_values("雙核勝率", ascending=False),
                                hide_index=True
                            )
        ...
        if st.button("🔄 執行即時對撞比對", key="run_clash_realtime_btn"):
            ...
            if snap and 'data' in snap:
                ...
                if not valid_items:
                    ...
                    probs      = brain.predict_win_rates(bulk_features)
                    candidates = sorted(
                        [{**item, 'win_prob': probs[i]} for i, item in enumerate(valid_items)],
                    ...
                    if not top_candidates:
                        ...
                        def fetch_clash(item):
                            ticker     = str(item.get('代號', '')).split('.')[0].strip()
                            stock_name = snap_dict.get(ticker, {}).get('名稱', current_names.get(ticker, ticker))
    ...
    with tab6:
        ...
        with st.spinner("正在從資料庫拉取歷史特徵並進行回測運算..."):
            ...
            if status == "ready":
                ...
                st.line_chart(
                    pd.DataFrame(res_adv['equity']).set_index("date_str")[["strat_cum_pct", "market_cum_pct"]]
                )
            elif status == "no_key":
