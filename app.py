...
if target_ticker:
    ...
    # ==========================================================
    # 🏠 滿血復活的主頁面 6 大頁籤 (包含 LSTM 加權全市場排序)
    # ==========================================================
    st.markdown("### 🌍 大盤與情緒摘要")
    ...
    with tab2:
        ...
        st.caption("本頁籤會自動加總全市場最新收盤落庫之 SMC 與技術面數據，由 AI 大腦算出明日全市場潛在多頭動能比率。")
        snapshot = load_market_snapshot()
        if snapshot:
            ...
            if model and bulk_features:
                try:
                    ...
                    bullish_ratio = float(np.mean(all_probs >= 0.52)) * 100
                    st.write("")
                    ...
                    else: st.error("❄️ **大腦趨勢研判：** 全市場潛在多頭動能極度萎縮，強烈建議啟動自動空倉防守。")
                except: st.info("趨勢矩陣計算中...")
            else: st.info("等待 AI 模型載入...")
            ...
            if model and bulk_features:
                try:
                    ...
                    for idx, item in enumerate(valid_items):
                        ...
                        take_profit = round(res_level + box_height, 2) if entry_price > res_level else (round(res_level, 2) if ("量縮回踩" in pattern_str) else round(res_level + (atr_14 * 1.0), 2))
                        profit_reason = "🚀 噴發目標" if entry_price > res_level else "🎯 潛伏/波段目標"
                        
                        box_color = "#00cc96" if final_prob > 0.52 else ("#ffc107" if final_prob > 0.50 else "#a8a8a8")
                        text_color = "#00cc96" if final_prob > 0.52 else ("#ffc107" if final_prob > 0.50 else "#f0f0f0")
                        ai_rec = "⭐⭐⭐ 雙核極致期望值！" if final_prob > 0.52 else ("⭐⭐ 溫和佈局" if final_prob > 0.50 else "⚠️ 建議嚴格觀望")
                        
                        processed_stocks.append({
                            'ticker': ticker, 'name': name, 'win_prob': final_prob, 'box_color': box_color, 'text_color': text_color, 'ai_rec': ai_rec,
    ...
    with tab5:
        ...
        st.caption("本面板會自動提取昨日收盤算出之 AI 預測信心度，與今日盤中「最新即時報價」直接跨時空對撞，讓洗盤與跳空無所遁形。")
        snapshot = load_market_snapshot()
        if snapshot:
            raw_list = snapshot['data']
            model, features = get_ai_model()
            ...
            if model and bulk_features:
                try:
                    ...
                    def fetch_live_comparison(idx_item):
                        ...
                        return {"股票代號": item['ticker'], "股票名稱": item['name'], "雙核預測勝率": f"{prob_pct:.1f}%", "戰術方向": direction, "昨收價": f"{y_close:.2f}", "即時現價": f"{rt_p:.2f}", "即時漲跌幅": f"{change_pct:+.2f}%", "實況比對": status}
        ...
        if res_adv["status"] == "no_key": st.error("⚠️ 找不到資料庫金鑰。")
            ...
            with sub_tab1:
                ...
                def build_card(title, value, subtext, color):
                    return f"""<div style="background-color: #121218; padding: 22px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #2a2a35; box-shadow: 0 4px 6px rgba(0,0,0,0.2);"><div style="color: #8b8b99; font-size: 14px; margin-bottom: 8px; font-weight: 500;">{title}</div><div style="color: {color}; font-size: 32px; font-weight: 700; margin-bottom: 5px;">{value}</div><div style="color: #6b6b79; font-size: 13px;">{subtext}</div></div>"""
                ...
                with r1_c2: st.markdown(build_card("帳戶實質總淨利", f"+${res_adv['net_profit_twd']:,.0f}" if res_adv['net_profit_twd'] > 0 else f"${res_adv['net_profit_twd']:,.0f}", f"初始本金 NT$ {user_capital:,}", color_green if res_adv['net_profit_twd'] > 0 else color_red), unsafe_allow_html=True)
                with r1_c3: st.markdown(build_card("帳戶真實報酬率", f"+{res_adv['account_pct']:.2f}%" if res_adv['account_pct'] > 0 else f"{res_adv['account_pct']:.2f}%", f"總成交筆數: {res_adv['trades']} 筆", color_green if res_adv['account_pct'] > 0 else color_red), unsafe_allow_html=True)
                
                r2_c1, r2_c2 = st.columns(2)
                with r2_c1: st.markdown(build_card("單筆期望值純利 (TWD)", f"+${res_adv['avg_trade_twd']:,.0f}" if res_adv['avg_trade_twd'] > 0 else f"${res_adv['avg_trade_twd']:,.0f}", "已扣除 0.5% 摩擦成本", color_green if res_adv['avg_trade_twd'] > 0 else color_red), unsafe_allow_html=True)
                with r2_c2: st.markdown(build_card("TP2 觸及概率 (波段發動)", f"{res_adv['tps']['tp2']*100:.1f}%", f"{res_adv['tps']['samples']} 筆樣本", color_purple), unsafe_allow_html=True)
